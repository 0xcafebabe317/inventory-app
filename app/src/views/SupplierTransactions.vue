...
import { getSupplierTransactions, updatePurchaseInvoice } from '../api/purchase'
...
async function loadTransactions() {
  ...
  try {
    ...
    const res: any = await getSupplierTransactions(supplierId, params)
    orders.value = res.data?.list || []
    total.value = res.data?.total || 0
    supplier.value = res.data?.supplier || null
  } catch { /* */ }
...
async function onSupplementUpload(orderId: number, e: Event) {
  ...
  try {
    const res: any = await uploadInvoice(file, 'purchase')
    const url = res.data?.url || ''
    if (url) {
      await updatePurchaseInvoice(orderId, url)
      // Update local state
...
<template>
  <div class="page">
    <van-nav-bar title="进货记录" left-text="返回" left-arrow @click-left="$router.back()" />

    <!-- Supplier Header -->
    <div class="hero-banner" v-if="supplier">
      ...
      <div class="hero-content">
        ...
        <div class="hero-info">
          ...
          <div class="hero-meta">
            {{ supplier.contact_name || '' }}{{ supplier.contact_name && supplier.phone ? ' · ' : '' }}{{ supplier.phone || '' }}
          </div>
        </div>
        <div class="hero-stat">
          <div class="hero-num">{{ total }}</div>
          <div class="hero-label">笔进货</div>
        </div>
      </div>
    </div>
    ...
    <van-empty v-if="!loading && !orders.length" description="暂无进货记录">
      <span style="color:#969799;font-size:13px">该进货商还没有任何进货记录</span>
    </van-empty>

    <!-- Transaction List -->
    <div v-for="(order, idx) in orders" :key="order.id" class="order-card" :style="{ animationDelay: `${idx * 0.05}s` }">
      <!-- Date & Total Header -->
...
.hero-banner {
  ...
  background: linear-gradient(145deg, #1e3a5f, #1a56db, #2563eb);
  overflow: hidden;
}
.hero-glow {
...
.hero-num { font-size: 28px; font-weight: 800; font-variant-numeric: tabular-nums; }
...
.order-date { font-size: 13px; color: #64748b; display: flex; align-items: center; gap: 4px; }
.date-icon { font-size: 14px; }
.order-total { text-align: right; }
.total-label { font-size: 11px; color: #94a3b8; display: block; }
.total-num { font-size: 20px; font-weight: 800; color: #1a56db; }
