...
import { getSale, refundSale } from '../api/sale'
...
async function loadOrder() {
  loading.value = true
  try {
    const res: any = await getSale(saleId)
...
async function handleRefund() {
  try {
    await showConfirmDialog({ title: '确认退货', message: '确定要为此订单办理退货吗？退货后库存将恢复。' })
  ...
  <div class="page">
    ...
    <template v-if="!loading && order.id">
      ...
      <div class="card">
        ...
        <div class="info-row">
          <span>收款方式：{{ payMethodLabel(order.pay_method) }}</span>
      ...
      <div class="card">
        ...
        <div v-for="item in order.items" :key="item.id" class="item-row">
          <div class="item-name">
            ...
            <span v-if="item.product?.supplier" class="item-supplier">· {{ item.product.supplier.name }}</span>
          </div>
...
.item-supplier { font-size: 12px; color: #4a90d9; font-weight: 400; }
