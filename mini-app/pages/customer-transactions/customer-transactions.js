...
Page({
  ...
  onLoad(options) {
    this.setData({ customerId: Number(options.id) })
  ...
  loadTransactions(overrides = {}) {
    ...
    request('/api/customers/' + this.data.customerId + '/transactions', 'GET', params).then(res => {
          ...
          success: (uploadRes) => {
            try {
              ...
              if (data.code === 'OK' && data.data.url) {
                request('/api/sale-orders/' + orderId + '/invoice', 'PUT', { invoice_url: data.data.url }).then(() => {
